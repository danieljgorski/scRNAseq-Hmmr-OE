...
Cells_preQC <- length(colnames(obj))
...
p <- VlnPlot(obj,
             ...
             group.by = "sample",
             pt.size=0)
pdf(file = "results/quality-control/VlnPlot_QC_metrics_pre-filter.pdf",
    width = 12,
    height = 6,
    pointsize = 12,
    useDingbats = F)
...
Cells_postQC <- length(colnames(obj))
Percent_passed <- (Cells_postQC/Cells_preQC)*100
QC_filter <- as.data.frame(Cells_preQC)
QC_filter$Cells_postQC <- Cells_postQC
QC_filter$Percent_passed <- Percent_passed
write.csv(QC_filter,
          file = "results/quality-control/QC_filter.csv",
          row.names = F)
...
p <- VlnPlot(obj,
             ...
             group.by = "sample",
             pt.size=0)
pdf(file = "results/quality-control/VlnPlot_QC_metrics_post-filter.pdf",
    width = 12,
    height = 6,
    pointsize = 12,
    useDingbats = F)
