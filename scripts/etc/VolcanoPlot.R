...
# colored red (up-regulated) and blue (down-regulated). The top 10 most
# statistically significant and top 10 most highly regulated genes
...
VolcanoPlot <- function(df, identity, title) {
  ...
  deg_up_sig_fc_topsig <- deg_up_sig_fc %>% slice_min(p_val_adj, n = 15)
  deg_up_sig_fc_topfc <- deg_up_sig_fc %>% slice_max(avg_log2FC, n = 15)
  deg_up_tops <- rbind(deg_up_sig_fc_topsig, deg_up_sig_fc_topfc)
  deg_up_tops <- distinct(deg_up_tops)
  deg_up_tops <- distinct(deg_up_tops)
  
  # separate highly overexpressed transgenes, to allow distinct label repel
  deg_up_Hmmr <- deg_up_tops[deg_up_tops$gene=="Hmmr",]
  deg_up_FIJ5 <- deg_up_tops[deg_up_tops$gene=="FIJ5",]
  deg_up_tops <- deg_up_tops[deg_up_tops$gene!="Hmmr",]
  deg_up_tops <- deg_up_tops[deg_up_tops$gene!="FIJ5",]
  ...
  deg_down_sig_fc_topsig <- deg_down_sig_fc %>% slice_min(p_val_adj, n = 15)
  deg_down_sig_fc_topfc <- deg_down_sig_fc %>% slice_min(avg_log2FC, n = 15)
